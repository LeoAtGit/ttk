...
int ttkTrackingGraph::GeneratePlanarTrackingGraph(
  vtkPolyData *output, vtkMultiBlockDataSet *correspondences) {
  ttk::Timer timer;
  ...
  for(int t = 0; t < nSteps; t++) {
    auto correspondencesPC
      = vtkImageData::SafeDownCast(correspondences->GetBlock(t));
    ...
    switch(matrix->GetDataType()) {
      vtkTemplateMacro((countEdges<VTK_TT, int>(
        nEdges, ttkUtils::GetPointer<VTK_TT>(matrix), dim)));
    }
  }
  ...
  for(int t = 0; t < nSteps; t++) {
    ...
    switch(matrix->GetDataType()) {
      vtkTemplateMacro((generateEdges<VTK_TT, int>(
        output, iEdge, outputCD, correspondencesPC->GetPointData(),
        ttkUtils::GetPointer<VTK_TT>(matrix), pointIndexOffset[t + 0],
        pointIndexOffset[t + 1], dim)));
    }
  }
