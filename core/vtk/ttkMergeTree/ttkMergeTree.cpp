...
int ttkMergeTree::RequestData(
    ...
    // Init segmentation
    auto segmentationIds = vtkSmartPointer<vtkDataArray>::Take( orderArray->NewInstance() );
    ...
    std::vector<const ttk::Propagation<ttk::SimplexId>*> sortedPropagations;
    {
        ...
        ttkVtkTemplateMacro(
            ...
            triangulation->getType(),
            (
                status = this->finalizePropagations<VTK_TT, ttk::SimplexId, TTK_TT>(
                    sortedPropagations,
                    ttkUtils::GetPointer<ttk::SimplexId>(segmentationIds),
                    propagations,
    ...
    // Finalize Output
    {
        ttk::Timer timer;
        this->printMsg(
            "Generating Output Data Objects",0,0,
        ...
        // Compute merge tree output
        {
          ...
          std::unordered_map<int,std::pair<int,int>> vertexToNodeMap; // vertexId -> NodeId,BranchId

          // build map
          {
            for(int p=0; p<nPropagations; p++){
              vertexToNodeMap.insert({sortedPropagations[p]->criticalPoints[0], {p, p}});
            }
          ...
          // nodes
          {
            ...
            auto pointCoordData = static_cast<float*>(ttkUtils::GetVoidPointer( pointCoords ));

            auto vertexId = vtkSmartPointer<vtkIntArray>::New();
            vertexId->SetName("VertexId");
            vertexId->SetNumberOfTuples(nNodes);
            auto vertexIdData = static_cast<int*>(ttkUtils::GetVoidPointer( vertexId ));

            auto branchId = vtkSmartPointer<vtkIntArray>::New();
            branchId->SetName( "BranchId" );
            branchId->SetNumberOfTuples(nNodes);
            auto branchIdData = static_cast<int*>(ttkUtils::GetVoidPointer( branchId ));

            auto outputScalars = vtkSmartPointer<vtkDataArray>::Take(scalarArray->NewInstance());
          ...
          // edges
          {
            ...
            auto offsetsData = static_cast<vtkIdType*>(ttkUtils::GetVoidPointer( offsets ));
            for(int i=0; i<=nEdges; i++)
              offsetsData[i] = i*2;

            auto connectivity = vtkSmartPointer<vtkIdTypeArray>::New();
            connectivity->SetNumberOfTuples( nEdges*2 );
            auto connectivityData = static_cast<vtkIdType*>(ttkUtils::GetVoidPointer( connectivity ));

            auto edgebranchId = vtkSmartPointer<vtkIntArray>::New();
            edgebranchId->SetName( "BranchId" );
            edgebranchId->SetNumberOfTuples(nEdges);
            auto edgebranchIdData = static_cast<int*>(ttkUtils::GetVoidPointer( edgebranchId ));
            ...
            auto edgePersistenceData = static_cast<double*>(ttkUtils::GetVoidPointer( edgePersistence ));
            ...
            auto typeData = static_cast<signed char*>(ttkUtils::GetVoidPointer( type ));
            ...
            for(int p=0, i=0, j=0; p<nPropagations; p++){
              ...
              double persistence = std::abs(
                (*scalarArray->GetTuple(criticalPoints[0])) - (*scalarArray->GetTuple(criticalPoints[nCriticalPoints-1]))
              );
        ...
        this->printMsg(
            "Generating Output Data Objects",1,timer.getElapsedTime()
        );
