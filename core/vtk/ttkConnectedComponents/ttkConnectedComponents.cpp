...
int ttkConnectedComponents::RequestData(vtkInformation *request,
  ...
  std::vector<ttk::ConnectedComponents::Component> components;
  {
    ...
    ttkVtkTemplateMacro(
      ...
      (status = this->computeConnectedComponents<VTK_TT, TTK_TT>(
         components, ttkUtils::GetPointer<VTK_TT>(outputArray),
  ...
  // Components Output
  {
    ...
    // points
    {
      auto labelArray = vtkSmartPointer<vtkIntArray>::New();
      labelArray->SetName(inputArray->GetName());
      labelArray->SetNumberOfTuples(nComponents);
      auto labelArrayData = ttkUtils::GetPointer<int>(labelArray);
      ...
      outputComponents->GetPointData()->AddArray(labelArray);
