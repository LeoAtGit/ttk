/// \ingroup base
/// \class ttk::TrackingFromGradient
...
namespace ttk {
  class TrackingFromGradient: virtual public Debug {

    public:
    TrackingFromGradient() {
      this->setDebugMsgPrefix("TrackingFromGradient");
    };
    ~TrackingFromGradient() {};
    ...
    int computeCorrelations(
      ...
      ttk::Timer timer;
      this->printMsg("Computing Correlations", 0, 0, this->threadNumber_,
      ...
      for(IT i = 0; i < nFeatures0; i++) {
        ...
        for(IT j=0; j<nFeatures1; j++){
          if(criticalPointVertexIds1[j]==matchIdx){
            correlations[ indexFunction(i,j,nFeatures0,nFeatures1) ]=1;
            found = true;
            break;
          }
        }
        if(!found)
          this->printErr("Unable to find matched vertex in given list of extrema.");
      }

      this->printMsg("Computing Correlations",
        1, timer.getElapsedTime(), this->threadNumber_);

      return 1;
    };

  }; // TrackingFromGradient class
