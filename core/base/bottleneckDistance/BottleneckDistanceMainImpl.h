...
int ttk::BottleneckDistance::computeBottleneck(
  ...
  double geometricalMinX = doubleMax;
  double geometricalMaxX = doubleMin;
  double geometricalMinY = doubleMax;
  double geometricalMaxY = doubleMin;
  double geometricalMinZ = doubleMax;
  double geometricalMaxZ = doubleMin;
  ...
  for(const auto &a : CTDiagram1) {
    ...
    if(geometricalMinX > minX)
      geometricalMinX = minX;
    if(geometricalMinY > minY)
      geometricalMinY = minY;
    if(geometricalMinZ > minZ)
      geometricalMinZ = minZ;
    if(geometricalMaxX < maxX)
      geometricalMaxX = maxX;
    if(geometricalMaxY < maxY)
      geometricalMaxY = maxY;
    if(geometricalMaxZ < maxZ)
      geometricalMaxZ = maxZ;
  }
  ...
  std::function<dataType(const diagramTuple)> diagonalDistanceFunction
    = [wasserstein, px, py, pz, ps, pe,
       maxDiag](const diagramTuple a) -> dataType {
    BNodeType ta1 = std::get<1>(a);
    BNodeType ta2 = std::get<3>(a);
    ...
    if(Geometry::pow(geoDistance, 1. / w) > maxDiag)
      geoDistance = Geometry::pow(maxDiag, w);
