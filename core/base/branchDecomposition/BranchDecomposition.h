...
/// Jonas Lukasczyk, Christoph Garth, Gunther H. Weber, Tim Biedert, Ross Maciejewski, and Heike Leitte.
/// IEEE Transactions on Visualization and Computer Graphics, Vol. 26, No. 1, 2020.
...
namespace ttk {
  ...
  public:
    ...
    int computeBranchDecompositionByAttribute(
      int* branchId,
      ...
      {
        ttk::Timer timer;
        const std::string msg = "Sorting Nodes by Time (#"+std::to_string(nNodes)+")";
      ...
      // Sorting Edges by attribute
      {
        ...
        const auto compareAttributeU = [=](const ttk::TrackingGraph::Edge& a, const ttk::TrackingGraph::Edge& b){return attribute[a.u]>attribute[b.u];};
        const auto compareAttributeV = [=](const ttk::TrackingGraph::Edge& a, const ttk::TrackingGraph::Edge& b){return attribute[a.v]>attribute[b.v];};
        const auto compareAttributeE = [=](const ttk::TrackingGraph::Edge& a, const ttk::TrackingGraph::Edge& b){return attribute[a.e]>attribute[b.e];};
      ...
      // Initializing Branches
      {
        ...
        #pragma omp parallel for num_threads(this->threadNumber_)
        for(int i=0; i<nNodes; i++){
          maxPrevNode[i] = trackingGraph.inEdges[i].size()>0 ? trackingGraph.inEdges[i][0].u : -1;
          maxNextNode[i] = trackingGraph.outEdges[i].size()>0 ? trackingGraph.outEdges[i][0].v : -1;
