  ...
  public:
    ...
    Triangle *buildTriangleList(const float *coords,
      ...
      for(size_t ti = 0; ti < nTriangles; ti++) {
        ...
        IT v1 = connectivityList[ti * 3 + 0];
        IT v2 = connectivityList[ti * 3 + 1];
        IT v3 = connectivityList[ti * 3 + 2];
        v1 *= 3;
        v2 *= 3;
        v3 *= 3;

        float x1 = coords[v1 + 0];
        float x2 = coords[v2 + 0];
        float x3 = coords[v3 + 0];

        float y1 = coords[v1 + 1];
        float y2 = coords[v2 + 1];
        float y3 = coords[v3 + 1];

        float z1 = coords[v1 + 2];
        float z2 = coords[v2 + 2];
        float z3 = coords[v3 + 2];

        float pMin[3] = {std::min({x1, x2, x3}), std::min({y1, y2, y3}),
                         std::min({z1, z2, z3})};
        float pMax[3] = {std::max({x1, x2, x3}), std::max({y1, y2, y3}),
                         std::max({z1, z2, z3})};
    ...
    bool MollerTrumbore(Ray &ray,
      ...
      CinemaImagingMath::subVectors(v0v1, &vertexCoords[v0], &vertexCoords[v1]);
      CinemaImagingMath::subVectors(v0v2, &vertexCoords[v0], &vertexCoords[v2]);

      CinemaImagingMath::cross(pvec, ray.m_direction, v0v2);
      float det = CinemaImagingMath::dot(v0v1, pvec);
      if(det > -kEpsilon && det < kEpsilon)
        return false;

      float invDet = 1.0f / det;

      CinemaImagingMath::subVectors(tvec, &vertexCoords[v0], ray.m_origin);
      float u = CinemaImagingMath::dot(tvec, pvec) * invDet;
      if(u < 0.0 || u > 1.0)
        return false;

      CinemaImagingMath::cross(qvec, tvec, v0v1);
      float v = CinemaImagingMath::dot(ray.m_direction, qvec) * invDet;
      if(v < 0.0 || u + v > 1.0)
        return false;

      float t = CinemaImagingMath::dot(v0v2, qvec) * invDet;
      ray.distance = t;
